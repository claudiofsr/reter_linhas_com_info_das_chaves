...
use crate::{
    ...
    get_modelo_documentos_fiscais, validar_chave_acesso,
};
...
pub fn imprimir_versao_do_programa() {
    ...
    let date = "3 de Fevereiro de 2026 (inicio: 01 de Outubro de 2018)";
    let version = "0.80";
...
/// Valida, remove do conjunto e salva chaves inválidas em streaming para o disco.
///
/// Otimizações:
/// - Zero alocação de vetores intermediários (O(1) extra memory).
/// - Buffers de 1MB para reduzir syscalls de I/O.
/// - Saneamento in-place no HashSet original.
pub fn processar_saneamento_e_salvar_chaves_invalidas(
    keys_doc: &mut HashSet<String>,
) -> SpedResult<()> {
    let path_nfe = "chaves_invalidas-nfe.txt";
    let path_cte = "chaves_invalidas-cte.txt";

    // 1. Abertura dos arquivos e configuração de buffers de 1MB
    let mut w_nfe = BufWriter::with_capacity(1024 * 1024, File::create(path_nfe)?);
    let mut w_cte = BufWriter::with_capacity(1024 * 1024, File::create(path_cte)?);

    let mut count_nfe = 0;
    let mut count_cte = 0;

    // Captura apenas o primeiro erro de I/O para reportar ao final
    let mut first_io_error: Option<std::io::Error> = None;

    // 2. Saneamento funcional via 'retain'
    keys_doc.retain(|chave| {
        let modelo = &chave[20..22];

        // Validamos apenas se for NFe (55) ou CTe (57)
        if (modelo == "55" || modelo == "57") && validar_chave_acesso(chave).is_err() {
            let res = if modelo == "55" {
                count_nfe += 1;
                writeln!(w_nfe, "{}", chave)
            } else {
                count_cte += 1;
            ...
            if let Err(e) = res
                && first_io_error.is_none()
            {
                first_io_error = Some(e);
            }
            return false; // Remove do HashSet
        }
        true // Mantém no HashSet
    });

    // 3. Propagação de erro de escrita, se houver
    if let Some(e) = first_io_error {
        return Err(SpedError::Io(e));
    }

    // 4. Esvaziar buffers antes de fechar os arquivos
    w_nfe.flush()?;
    w_cte.flush()?;

    // 5. Finalização: Estatísticas e limpeza de arquivos vazios
    finalizar_auditoria_invalidas(count_nfe, path_nfe, "NFe")?;
    finalizar_auditoria_invalidas(count_cte, path_cte, "CTe")?;

    if count_nfe == 0 && count_cte == 0 {
        println!(
            " ✅ Integridade validada: Todas as chaves NFe/CTe possuem dígito verificador correto."
        );
    }
...
/// Helper para reportar resultados e remover arquivos que ficaram vazios.
fn finalizar_auditoria_invalidas(count: usize, path: &str, label: &str) -> SpedResult<()> {
    if count > 0 {
        println!(
            " ❌ Detectadas {:>6} chaves de {} inválidas. Relatório gerado em: <{}>",
            fmt_milhares(count),
            label,
            path
        );
    } else {
        // Remove o arquivo físico caso não tenha sido encontrada nenhuma falha
