...
use reter_linhas_com_info_das_chaves::{
    ...
    processar_saneamento_e_salvar_chaves_invalidas, read_csv_files,
};
...
fn run() -> SpedResult<()> {
    ...
    // 8. Processamento EFD
    // Tornamos a variável mutável para permitir a filtragem posterior
    let mut keys_efd = get_efd_info(&config)?;

    println!(
        "\nAnalisando o arquivo SPED EFD Contribuições: {:?}\n",
        config.efd_path
    );

    // 11. Saneamento de Integridade
    // Esta função agora valida, exporta os erros e limpa o HashSet original
    // garantindo que as próximas etapas trabalhem apenas com dados íntegros.
    processar_saneamento_e_salvar_chaves_invalidas(&mut keys_efd)?;
    ...
    // 13. Relatório Final de Ausências
